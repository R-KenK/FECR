...
From these values, one can calculate FECR values through the `FECR()` function of this `FECR`
package, specifying a method between `"Kochapakdee"`, `"Dash"`, `"Coles"`, `"Cabaret1"`,
`"Cabaret2"`.    
The logical argument `compute.CI` allows for the computation of a confidence interval 
(95% by default) for individual-based methods (cf. Cabaret & Berrag, 2004)
...
```
Note that shuffling obviously didn't change EPG distribution in T and C groups:
...
And neither are methods based on group-averaged EPG
...
CI.shuffled <- replicate(
  ...
  expr = {
    ...
    C1.new <- C1[new.C]
    C2.new <- C2[new.C]
    Cab <- invisible(FECR(T1.new,T2.new,C1.new,C2.new,method = "Cabaret2",compute.CI = TRUE))
    Mac <- invisible(FECR(T1.new,T2.new,C1.new,C2.new,method = "MacIntosh",compute.CI = FALSE))
    
    
    Mac <- FECR(T1.new,T2.new,C1.new,C2.new,method = "MacIntosh",compute.CI = FALSE)
