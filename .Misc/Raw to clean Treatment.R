...
Trt.raw[season=="fall",season:="1.fall"]
Trt.raw[season=="winter",season:="2.winter"]
Trt.raw[season=="spring",season:="3.spring"]
Trt.raw[season=="summer",season:="4.summer"]

# clean the season factor's levels
droplevels(Trt.raw$season)
Trt.raw$date<- as.Date(Trt.raw$date,origin="1970-01-01")
...
Trt.control<- Trt.raw[,.(date=mean(date),id="control",drug="control"),by=.(year,season,seasonchron,dose_no)][,ordered.col.to.keep,with=FALSE]
